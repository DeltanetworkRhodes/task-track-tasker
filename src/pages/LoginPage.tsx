      ...
      <div className="relative w-full max-w-sm space-y-6 rounded-2xl border border-border bg-card p-8 shadow-xl shadow-primary/5">
        ...
        {mode === "forgot" && forgotSent ? (
          <div className="text-center space-y-3 py-4">
            <p className="text-sm text-foreground font-medium">📧 Email στάλθηκε!</p>
            <p className="text-xs text-muted-foreground">Ελέγξτε το inbox σας για τον σύνδεσμο επαναφοράς κωδικού.</p>
            <button
              onClick={() => { setMode("login"); setForgotSent(false); setError(""); }}
              className="text-primary text-xs font-semibold hover:underline flex items-center gap-1 mx-auto"
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === "signup" && (
              <>
                ...
                <div>
                  <label className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Κινητό</label>
                  <input
                    type="tel"
                    value={phone}
        ...
        <p className="text-center text-xs text-muted-foreground">
          {mode === "forgot" && !forgotSent ? (
            <button onClick={() => { setMode("login"); setError(""); }} className="text-primary font-semibold hover:underline flex items-center gap-1 mx-auto">
              <ArrowLeft className="h-3 w-3" /> Πίσω στη Σύνδεση
            </button>
          ) : mode === "login" ? (
            <>Δεν έχεις λογαριασμό;{" "}
              <button onClick={() => { setMode("signup"); setError(""); }} className="text-primary font-semibold hover:underline">Εγγραφή</button>
            </>
          ) : mode === "signup" ? (
            <>Έχεις ήδη λογαριασμό;{" "}
              <button onClick={() => { setMode("login"); setError(""); }} className="text-primary font-semibold hover:underline">Σύνδεση</button>
            </>
          ) : null}
        </p>
      </div>

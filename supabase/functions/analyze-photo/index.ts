...
ΒΗΜΑ 2 – ΑΞΙΟΛΟΓΗΣΗ ΠΟΙΟΤΗΤΑΣ (score 1-10):
  ΠΕΡΝΑΜΕ ΣΧΕΔΟΝ ΟΛΑ. Απόρριψη ΜΟΝΟ για κακοτεχνίες ή ΕΝΤΕΛΩΣ λάθος κατηγορία.
  9-10: Πολύ καλή δουλειά
  7-8: Κανονική δουλειά, τα βασικά υπάρχουν (ΑΥΤΟ ΕΙΝΑΙ ΤΟ ΣΥΝΗΘΕΣ)
  6: Μέτρια αλλά αποδεκτή
  4-5: Κακοτεχνία – σοβαρό πρόβλημα που πρέπει να διορθωθεί
  1-3: Λάθος κατηγορία ή εντελώς άσχετη φωτογραφία
  
  ΣΗΜΑΝΤΙΚΟ: ΕΓΚΡΙΝΕ ΤΗΝ αν δείχνει τη σωστή φάση. Score >= 6 = ΕΓΚΡΙΣΗ. Μην ψάχνεις ελαττώματα – ψάξε μόνο ΚΑΚΟΤΕΧΝΙΕΣ.

ΒΗΜΑ 3 – CONTEXTUAL VALIDATION:
  ΑΠΟΡΡΙΨΗ ΜΟΝΟ σε αυτές τις περιπτώσεις:
  • Εντελώς ΛΑΘΟΣ κατηγορία (π.χ. ανεβάζει "Αποκατάσταση" αλλά βλέπεις ανοιχτή τρύπα)
  • ΚΑΚΟΤΕΧΝΙΑ: σπασμένοι σωλήνες, μπερδεμένα καλώδια, κατεστραμμένος εξοπλισμός
  • Εντελώς ΑΣΧΕΤΗ φωτογραφία (selfie, φαγητό κλπ)
  
  ΟΛΑ ΤΑ ΑΛΛΑ ΕΓΚΡΙΝΟΝΤΑΙ – ακόμα κι αν η δουλειά δεν είναι τέλεια. Λίγο ακατάστατο = ΟΚ (score 7). Λείπει ταινία = ΟΚ (score 7, σχόλιο). Ελαφρά στραβό = ΟΚ.
...
ΚΡΙΤΗΡΙΑ ΠΟΙΟΤΗΤΑΣ (λογική, ΟΧΙ τελειομανία):
- Τα κανάλια πρέπει να είναι ΣΧΕΤΙΚΑ ΙΣΙΑ – δεν χρειάζεται τελειότητα, αρκεί να φαίνεται ότι υπήρξε προσπάθεια
- Τα σπιράλ πρέπει να είναι ΛΟΓΙΚΑ ΠΕΡΑΣΜΕΝΑ – χωρίς μεγάλα κρεμάσματα ή μπερδέματα
- Η δουλειά πρέπει να φαίνεται ΑΞΙΟΠΡΕΠΗΣ (δεν χρειάζεται να είναι τέλεια)
- ΜΟΝΟ πραγματικά τσαπατσούλικη δουλειά (ζιγκ-ζαγκ, μεγάλα κρεμάσματα) → score < 6

ΕΓΚΡΙΣΗ (score >= 6) αν φαίνεται τουλάχιστον ΕΝΑ στοιχείο όδευσης και η δουλειά δεν είναι τραγική.
ΑΠΟΡΡΙΨΗ (score < 6) ΜΟΝΟ αν δεν φαίνεται καμία σωλήνωση/κανάλι ή η εγκατάσταση είναι πραγματικά ΚΑΚΗ.`,
...
ΒΑΘΜΟΛΟΓΗΣΗ (ΝΑ ΕΙΣΑΙ ΛΟΓΙΚΟΣ, ΟΧΙ ΑΥΣΤΗΡΟΣ):
- 8-10: Σωστή κατηγορία + τα βασικά στοιχεία υπάρχουν + αξιοπρεπής δουλειά
- 6-7: Σωστή κατηγορία, μικρές ατέλειες, αποδεκτό – ΕΓΚΡΙΝΕΤΑΙ
- 4-5: Σωστή κατηγορία αλλά λείπουν ΒΑΣΙΚΑ πράγματα ή πολύ χαμηλή ποιότητα
- 1-3: Λάθος κατηγορία, θολή, ή εντελώς άσχετη

isApproved = true αν score >= 6 ΚΑΙ η φωτογραφία αντιστοιχεί στη σωστή κατηγορία.
ΣΗΜΑΝΤΙΚΟ: Μην απορρίπτεις φωτογραφίες για μικρολεπτομέρειες. Απόρριψη ΜΟΝΟ για πραγματικά λάθος κατηγορία ή πολύ κακή δουλειά.
        ...
        function: {
          ...
          parameters: {
            ...
            properties: {
              isApproved: {
                type: "boolean",
                description: "true if photo matches the correct category AND has quality score >= 6. Be reasonable, not perfectionist.",
              },
              qualityScore: {
                type: "number",
                description: "Quality score from 1 to 10. Score < 6 means rejected. Be generous for decent work.",
              },
              stageIdentified: {
                type: "string",

...
ΚΡΙΤΗΡΙΑ ΠΟΙΟΤΗΤΑΣ (ΠΟΛΥ ΕΛΑΣΤΙΚΑ):
- Αρκεί να φαίνεται ότι είναι σχετική δουλειά – ΔΕΝ χρειάζεται τελειότητα
- Ελαφρά στραβά κανάλια = ΟΚ (score 7)
- Μικρά κρεμάσματα = ΟΚ (score 7, σχόλιο)
- ΜΟΝΟ σοβαρή κακοτεχνία (ζιγκ-ζαγκ, τεράστια κρεμάσματα, σπασμένα) → score < 6

ΕΓΚΡΙΣΗ (score >= 6) αν φαίνεται τουλάχιστον ΕΝΑ στοιχείο όδευσης. Περνάμε σχεδόν τα πάντα.
ΑΠΟΡΡΙΨΗ (score < 6) ΜΟΝΟ αν δεν φαίνεται καμία σωλήνωση/κανάλι ή υπάρχει ΣΟΒΑΡΗ κακοτεχνία.`,
...
ΒΑΘΜΟΛΟΓΗΣΗ (ΠΕΡΝΑΜΕ ΣΧΕΔΟΝ ΟΛΑ – ΑΠΟΡΡΙΨΗ ΜΟΝΟ ΓΙΑ ΚΑΚΟΤΕΧΝΙΕΣ):
- 8-10: Σωστή κατηγορία + καλή δουλειά (ΤΟ ΣΥΝΗΘΕΣ)
- 6-7: Σωστή κατηγορία, μικρές ατέλειες – ΕΓΚΡΙΝΕΤΑΙ ΑΝΕΤΑ
- 4-5: Σωστή κατηγορία αλλά υπάρχει ΚΑΚΟΤΕΧΝΙΑ (σπασμένα, κατεστραμμένα, σοβαρά λάθη)
- 1-3: Λάθος κατηγορία ή εντελώς άσχετη

isApproved = true αν score >= 6 ΚΑΙ η φωτογραφία αντιστοιχεί στη σωστή κατηγορία.
ΣΗΜΑΝΤΙΚΟ: ΠΕΡΝΑ ΣΧΕΔΟΝ ΟΛΑ. Απόρριψη ΜΟΝΟ για κακοτεχνίες ή λάθος κατηγορία. Μικροελαττώματα = ΕΓΚΡΙΣΗ με σχόλιο.
